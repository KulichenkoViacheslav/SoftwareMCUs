...
static leds_state_s_t leds_state = {0};
...
void led_init(void)
{
	...
	HAL_GPIO_WritePin(LED_GREN_GPIO_Port, LED_GREN_Pin, GPIO_PIN_RESET);
...
void led_switch(led_choes_led_e_t choes_led, led_state_e_t state_led)
{
	...
	switch (choes_led)
  {
		case led_blue :
	  {
		...
		case led_green :
	  {
		 	HAL_GPIO_WritePin(LED_GREN_GPIO_Port, LED_GREN_Pin, GPIO_PIN_SET);	
